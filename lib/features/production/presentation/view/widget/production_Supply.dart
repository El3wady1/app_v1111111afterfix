...
class _Production_SupplyState extends State<Production_Supply> {
  ...
  String apiUrl = "${Apiendpoints.baseUrl}${Apiendpoints.orderSupply.getAll}";
  ...
  Future<void> _loadProductionRequests() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> filteredProductionData = productionData.where((item) {
          return item['isSend'] == false;
        }).toList();

        print("Found ${filteredProductionData.length} items with isSend: false");
        ...
        for (var item in filteredProductionData) {
          try {
            ...
            final productKey = '${item['product']['name']}-${item['package']}-${item['mainProductOP']['name']}-$branchName';
            
            if (combinedItems.containsKey(productKey)) {
              combinedItems[productKey]!.requestedQty += (item['qty'] as num?)?.toDouble() ?? 0.0;
            } else {
        ...
        setState(() {
          ...
          final uniqueCategories = items.map((item) => item.category).where((cat) => cat != null).toSet().toList();
          allCategories = uniqueCategories;
          
          // تحديث قائمة الفروع للفلترة مع تطبيق الترتيب المطلوب
          final uniqueBranches = items.map((item) => item.branch).where((branch) => branch != null).toSet().toList();
          ...
          branchDropdownItems = [
            ...
            ...allBranches.map((branch) => DropdownMenuItem<String>(
              value: branch,
              child: Text(branch),
            )).toList()
          ];
  ...
  void _filterItems() {
    ...
    setState(() {
      ...
      // تطبيق فلتر الفئات
      if (selectedCategories.isNotEmpty && selectedCategories.length != allCategories.length) {
        tempItems = tempItems.where((item) => selectedCategories.contains(item.category)).toList();
      }
      
      // تطبيق فلتر الفروع (Dropdown)
      if (selectedBranch != null) {
        tempItems = tempItems.where((item) => item.branch == selectedBranch).toList();
      }
  ...
  double _calculateTotalForProduct(String productName, Map<String, Map<String, ProductionItem>> organizedData) {
    double total = 0.0;
  ...
  double _calculateBranchTotal(String branchName, Map<String, Map<String, ProductionItem>> organizedData) {
