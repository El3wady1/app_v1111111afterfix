...
import 'package:inventory/features/production/presentation/view/widget/approval_Supply.dart';
import 'package:inventory/features/production/presentation/view/widget/production_Supply.dart';
import 'package:modal_progress_hud_nsn/modal_progress_hud_nsn.dart';
import 'package:flutter/material.dart' as dirc;
...
class _ProductionBodyViewState extends State<ProductionBodyView> {
  ...
  String apiUrl =
      "${Apiendpoints.baseUrl}${Apiendpoints.orderProduction.getAll}";
  String submitUrl =
  ...
  Future<void> _loadProductionRequests() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> filteredProductionData =
            productionData.where((item) {
  ...
  Future<void> _loadAcceptedRequests() async {
    ...
    try {
      final response =
          await http.get(Uri.parse(acceptedUrl)).timeout(Duration(seconds: 30));
  ...
  void _filterItems() {
    ...
    setState(() {
      ...
      if (selectedBranch != null) {
        tempItems =
            tempItems.where((item) => item.branch == selectedBranch).toList();
      }
  ...
  List<ProductionItem> _getAllUniqueProducts() {
    ...
    return uniqueProducts.values.toList()..sort((a, b) => a.name.compareTo(b.name));
  }

  // دالة جديدة لحساب الإجمالي لكل منتج عبر جميع الفروع
  double _calculateTotalForProduct(
      String productName, Map<String, Map<String, ProductionItem>> organizedData) {
    double total = 0.0;
  ...
  double _calculateBranchTotal(
      String branchName, Map<String, Map<String, ProductionItem>> organizedData) {
  ...
  double _calculateGrandTotal(Map<String, Map<String, ProductionItem>> organizedData) {
    double total = 0.0;
    for (var branchData in organizedData.values) {
  ...
  Widget _buildCategoryBasedView() {
    ...
    if (organizedData.isEmpty) {
      return Center(
        child: Text( "لا توجد نتائج للعرض".tr()),
      );
    }
  ...
  Widget _buildCategorySection(String category, List<ProductionItem> products, List<String> branches) {
  ...
  Map<String, Map<String, ProductionItem>> _organizeItemsByBranchAndProductForCategory(List<ProductionItem> products) {
    Map<String, Map<String, ProductionItem>> organizedData = {};
  ...
  // بناء جدول المنتجات
  Widget _buildProductsTable(Map<String, Map<String, ProductionItem>> organizedData, List<String> branches) {
    ...
    return Column(
      children: [
        ...
        ...uniqueProducts.map((product) => 
          _buildProductRow(product, branches, organizedData)
        ).toList(),
  ...
  List<ProductionItem> _getUniqueProductsFromOrganizedData(Map<String, Map<String, ProductionItem>> organizedData) {
  ...
  Widget _buildTableHeader(List<String> branches) {
    ...
    final branchColumnWidth = (availableWidth - productColumnWidth) / (branches.length + 1);
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text('${item.package} ${item.packageUnitname}',
                      style: GoogleFonts.cairo(
                        fontSize: isVerySmallScreen
                            ? 14
                            : isSmallScreen
  ...
  void _showBulkActionDialog(bool isApprove) {
    ...
    final pendingItems =
        acceptedItems.where((item) => item.status == "pending").toList();
                          ...
                          Expanded(
                            child: Container(
                              ...
                              child: MultiSelectDialogField(
                                ...
                                buttonIcon: Icon(Icons.category, color: primaryColor),
                                buttonText: Text(
                    ...
                    SizedBox(
                        height: isVerySmallScreen
                            ...
                            : isSmallScreen
                                ? 16
                                : 20),
                    Text('لا توجد طلبات في هذا القسم'.tr(),
                        style: GoogleFonts.cairo(
                          fontSize: isVerySmallScreen
                              ? 16
                              : isSmallScreen
              ...
              SizedBox(
                  height: isVerySmallScreen
                      ...
                      : isSmallScreen
                          ? 16
                          : 20),
              Text('هل أنت متأكد من الكميات المنتجة؟'.tr(),
                  style: GoogleFonts.cairo(
                    fontSize: isVerySmallScreen
                        ? 16
                        : isSmallScreen
                    ...
                    SizedBox(
                        width: isVerySmallScreen
                            ...
                            : isSmallScreen
                                ? 8
                                : 10),
                    Text('بعض الأصناف غير معتمدة'.tr(),
                        style: GoogleFonts.cairo(
                          fontSize: isVerySmallScreen
                              ? 14
                              : isSmallScreen
  ...
  Future<void> _submitProductionRequest() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('فشل في العملية'.tr(), style: GoogleFonts.cairo(fontSize: 16)),
          backgroundColor: Colors.red,
          behavior: SnackBarBehavior.floating,
        ),
      );
...
class ProductionItem {
  ...
  ProductionItem({
    ...
    required this.name,
    required String package, // تغيير هنا
    String? packageUnitname,
  ...
  }) : package = (package == "لم يحدد") ? "" : package, // إضافة التهيئة هنا
