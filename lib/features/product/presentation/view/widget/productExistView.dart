...
import 'package:inventory/features/in/data/services/addQtyAndExpiredProductServices.dart';
...
class _MultiBarcodeEntryViewState extends State<MultiBarcodeEntryView> {
  ...
  Future<void> _addExistingInventory(ProductEntry product) async {
    try {
      final response = await http.post(
      ...
      if (response.statusCode == 200||response.statusCode == 201) {
        ...
        if (data['message'] != "INEXIST transaction created successfully") {
          throw Exception(
              data['message'] ?? 'Failed to add existing inventory');
        }
      } else {
        throw Exception('Server error: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Failed to add existing inventory: $e');
    }
  }
  ...
  Future<void> _saveAllProducts() async {
    ...
    try {
      for (var product in products) {
        await _addExistingInventory(product);
