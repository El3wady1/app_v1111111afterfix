...
class LastsendproductionSupplyview extends StatefulWidget {
  ...
  @override
  State<LastsendproductionSupplyview> createState() => _LastsendproductionSupplyviewState();
}

class _LastsendproductionSupplyviewState extends State<LastsendproductionSupplyview> {
  ...
  Future<void> fetchHistoryData() async {
    ...
    try {
      final response = await http.get(Uri.parse(Apiendpoints.baseUrl + Apiendpoints.productionSupply.history));
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          selectedEndDate = DateTime(today.year, today.month, today.day, 23, 59, 59);
  ...
  void extractLastOrderByBranch() {
    ...
    for (var item in historyData) {
      ...
      if (!lastOrderByBranch.containsKey(branchId) || 
          DateTime.parse(item['createdAt']).isAfter(DateTime.parse(lastOrderByBranch[branchId]['createdAt']))) {
        lastOrderByBranch[branchId] = item;
      }
    }
  }

  Future<void> updateHistoryItem(String id, List<Map<String, dynamic>> items) async {
    try {
      final response = await http.put(
        Uri.parse('${Apiendpoints.baseUrl}${Apiendpoints.productionSupply.updatehistory}$id'),
  ...
  Future<void> deleteHistoryItem(String id) async {
    try {
      final response = await http.delete(
        Uri.parse('${Apiendpoints.baseUrl}${Apiendpoints.productionSupply.deletehistory}$id'),
      );
 ...
 void showDeleteConfirmationDialog(String id) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'هل أنت متأكد من أنك تريد حذف هذه العملية؟ لا يمكن التراجع عن هذا الإجراء.'.tr(),
            style: GoogleFonts.cairo(color: primaryColor),
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: Text('إلغاء'.tr(), style: GoogleFonts.cairo(color: primaryColor)),
            ),
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
              ...
              child: Text('حذف'.tr(), style:  GoogleFonts.cairo(color: Colors.white)),
            ),
          ],
        );
      },
    ...
    setState(() {
      filteredHistoryData = historyData.where((item) {
        ...
        if (selectedStartDate != null) {
          isAfterStartDate = createdAt.isAfter(
            DateTime(selectedStartDate!.year, selectedStartDate!.month, selectedStartDate!.day, 0, 0, 0).subtract( Duration(seconds: 1))
          );
        }
        
        if (selectedEndDate != null) {
          isBeforeEndDate = createdAt.isBefore(
            DateTime(selectedEndDate!.year, selectedEndDate!.month, selectedEndDate!.day, 23, 59, 59)
          );
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: primaryColor,
        title:  Text('سجل العمليات',style: GoogleFonts.cairo(color: Colors.white),).tr(),
        centerTitle: true,
        actions: [
          IconButton(
            icon: Icon(isFilterVisible ? Icons.filter_alt_off : Icons.filter_alt, color: Colors.white),
            onPressed: () {
              setState(() {
              ...
              : historyData.isEmpty
                  ? Center(
                      child: Text(
                        ...
                        style: GoogleFonts.cairo(fontSize: 18, color: primaryColor),
                      ),
                    )
                  : Column(
                      children: [
                              ...
                              children: [
                                                ...
                                                children: [
                                                  Text(
                                                    selectedStartDate != null
                                                        ...
                                                        : 'اختر التاريخ'.tr(),
                                                    style: GoogleFonts.cairo(
                                                      color: selectedStartDate != null ? primaryColor : Colors.grey,
                                                    ),
                                                  ),
                                                ...
                                                children: [
                                                  Text(
                                                    selectedEndDate != null
                                                        ...
                                                        : 'اختر التاريخ'.tr(),
                                                    style: GoogleFonts.cairo(
                                                      color: selectedEndDate != null ? primaryColor : Colors.grey,
                                                    ),
                                                  ),
                                                  Icon(Icons.calendar_today, size: 20, color: primaryColor),
                                                ],
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: ElevatedButton(
                                        ...
                                        child: Text('تطبيق الفلتر'.tr(), style:  GoogleFonts.cairo(color: Colors.white)),
                                      ),
                                    ),
                                  ],
                                ),
                                if (selectedStartDate != null || selectedEndDate != null)
                                  Padding(
                                    ...
                                    child: Text(
                                      'عدد النتائج'.tr()+" :"+'${filteredHistoryData.length}'.tr(),
                                      style: GoogleFonts.cairo(
                                        fontSize: 14,
                                        color: primaryColor,
                                        fontWeight: FontWeight.bold,
  ...
  children: [
    Expanded(
      child: Text(
        selectedStartDate != null || selectedEndDate != null
            ? 'عرض البيانات من ${selectedStartDate != null ? DateFormat('yyyy/MM/dd').format(selectedStartDate!) : 'البداية'.tr()} إلى ${selectedEndDate != null ? DateFormat('yyyy/MM/dd').format(selectedEndDate!) : 'النهاية'.tr()}'.tr()
    ...
    Text(
      'النتائج'.tr() + " : " + '${filteredHistoryData.length}',
      style: GoogleFonts.cairo(
        fontSize: 14,
        color: primaryColor,
        fontWeight: FontWeight.bold,
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              setState(() {
                                ...
                                filteredHistoryData = lastOrderByBranch.values.toList();
                              });
                            },
                            style: ElevatedButton.styleFrom(
                              ...
                              child: filteredHistoryData.isEmpty
                                  ? Center(
                                      child: Column(
                                        mainAxisAlignment: MainAxisAlignment.center,
                                        children: [
                                          Icon(Icons.search_off, size: 50, color: primaryColor.withOpacity(0.5)),
                                           SizedBox(height: 16),
                                          Text(
                                  ...
                                  : ListView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        final branchName = item['branch']?['name'] ?? 'غير محدد'.tr();
                                        ...
                                        return Container(
                                          ...
                                          child: ExpansionTile(
                                            ...
                                            title: Row(
                                              children: [
                                                Container(
                                                  ...
                                                  decoration: BoxDecoration(
                                                    color: _getActionColor(item['action']).withOpacity(0.2),
                                                    borderRadius: BorderRadius.circular(5),
                                                    border: Border.all(color: _getActionColor(item['action'])),
                                                  ),
                                                  child: Text(
                                            ...
                                            subtitle: Column(
                                              children: [
                                                Text(
                                                  'تاريخ التحديث: ${formatDate(item['updatedAt'])}'.tr(),
                                                  style: GoogleFonts.cairo(
                                                    fontSize: 12,
                                                    color: Colors.grey[600],
                                                  ),
                                                ...
                                                Padding(
                                                  ...
                                                  child: Text(
                                                    'تاريخ الإنشاء: ${formatDate(item['createdAt'])}'.tr(),
                                                    style: GoogleFonts.cairo(
                                                      fontSize: 12,
                                                      color: Colors.grey[600],
                                                    ),
                                                  ...
                                                  children: [
                                                    ...
                                                    Table(
                                                      ...
                                                      children: [
                                                        ...
                                                        // بيانات الجدول
                                                        ...item['items'].map<TableRow>((productItem) {
                                                          return TableRow(
                                                            children: [
                                                              Padding(
                                                                padding:  EdgeInsets.all(10.0),
