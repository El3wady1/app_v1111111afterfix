...
import 'package:inventory/features/send/presentation/view/widget/lastSendProductionSupply.dart'
    show LastsendproductionSupplyview;
import 'package:inventory/features/send/presentation/view/widget/lastSendView.dart';
import 'package:inventory/features/send/presentation/view/widget/sendSupplyview.dart';
...
class _SendbodyviewState extends State<Sendbodyview>
  ...
  final List<Widget> _screens = [
    ProductionBodyView(role: 'admin'), // سيتم استبدالها بالـ role الفعلي
...
class _ProductionBodyViewState extends State<ProductionBodyView> {
  ...
  List<AdditionalProduct> filteredAdditionalProducts = []; // المنتجات الإضافية المصفاة للفرع الحالي
  ...
  List<String> orderIds = []; // تغيير هنا: استخدام قائمة بدلاً من متغير واحد
  ...
  Future<void> _loadBranches() async {
    ...
    try {
      ...
      final response =
          await http.get(Uri.parse(branchesUrl)).timeout(Duration(seconds: 30));
      ...
      if (response.statusCode == 200) {
        ...
        for (var item in productionData) {
          try {
            ...
            final regex = RegExp(
                r'طلب انتاج - (\d{1,2})/(\d{1,2})/(\d{4}) - (\d{1,2}):(\d{1,2})');
        ...
        // Group items by branch and main product
        ...
        // Create a list with branch headers and their items
        List<ProductionItem> combinedItems = [];
        branchItemsMap.forEach((branch, items) {
          // Add branch header
          combinedItems.add(ProductionItem(
            id: branch,
            productId: '',
            name: branch,
            package: '',
          ...
          for (var item in items) {
            if (item.mainProductId.isNotEmpty) {
              if (!mainProductGroups.containsKey(item.mainProductId)) {
                mainProductGroups[item.mainProductId] = [];
              }
              mainProductGroups[item.mainProductId]!.add(item);
            } else {
              combinedItems.add(item);
            }
          }

          // Add main product groups
          mainProductGroups.forEach((mainProductId, subItems) {
            ...
            Expanded(
              ...
              child: Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(Icons.remove,
                        color: primaryColor, size: isVerySmallScreen ? 18 : 20),
                    padding: EdgeInsets.zero,
                    constraints: const BoxConstraints(),
                    onPressed: () {
                      if (!mounted) return;
                    ...
                    child: SizedBox(
                      ...
                      child: TextField(
                        ...
                        onChanged: (value) {
                          if (!mounted) return;
                          setState(() {
                            item.requestedQty = double.tryParse(value) ?? 0;
                          });
                        },
                      ),
                    ),
  ...
  Future<void> _submitProductionRequest() async {
    if (!mounted || selectedBranch == null || selectedBranch!.id == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    try {
      ...
      final itemsToSubmit = filteredItems
          .where((item) =>
              ...
              !item.isBranchHeader)
          .map(
              (item) => {"productId": item.productId, "qty": item.requestedQty})
          .toList();
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (orderIds.isNotEmpty) {
          ...
          print(
              "✅ تم تحديث $successCount طلب بنجاح، فشل في تحديث $failCount طلب");

          if (failCount > 0 && mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    'تم تحديث $successCount طلب بنجاح، ولكن فشل في تحديث $failCount طلب'
                        .tr()),
                backgroundColor: Colors.orange,
                behavior: SnackBarBehavior.floating,
              ),
                    ...
                    child: ListView(
                      children: [
                        ...
                        if (_hasRequestsForCurrentBranch)
                          ...filteredItems
                              .where((item) => !item.isBranchHeader && !item.isMainProduct)
                              .map((item) => _buildProductionItemRow(
...
Future updateOrderIsSended({
  ...
  final url = Uri.parse(
      "${Apiendpoints.baseUrl}${Apiendpoints.orderProduction.isSend}$orderId");
