...
class Lastorderproductionview extends StatefulWidget {
  final String role;
  var canedite;
  Lastorderproductionview({required this.role, required this.canedite});

  @override
  _LastorderproductionviewState createState() =>
      _LastorderproductionviewState();
}

class _LastorderproductionviewState extends State<Lastorderproductionview> {
  ...
  String BranchOPuser =
      "${Apiendpoints.baseUrl}${Apiendpoints.auth.userBranchOP}";
  String submitUrl =
      "${Apiendpoints.baseUrl + Apiendpoints.production.request}";
  String editUrl =
      "${Apiendpoints.baseUrl + Apiendpoints.orderProduction.update}";
  String addProductUrl =
      "${Apiendpoints.baseUrl + Apiendpoints.orderProduction.add}";
  String productsUrl =
  ...
  // دالة محسنة لتصفية المنتجات الموجودة في آخر طلب إنتاج للفرع المحدد
  List<AdditionalProduct> _filterExistingProducts(List<AdditionalProduct> allProducts) {
    ...
    // جمع معرفات المنتجات الموجودة في آخر طلب إنتاج للفرع المحدد فقط
...
bool _isCurrentBranchSent() {
  ...
  Future<void> _loadAdditionalProducts() async {
    ...
    try {
      final response = await http.get(Uri.parse(productsUrl)).timeout(Duration(seconds: 30));
            ...
            if (product['isorderProduction'] == true) {
              loadedProducts.add(AdditionalProduct(
                ...
                packageUnitName: product['packageUnit'] is Map
                    ? (product['packageUnit']['name']?.toString() ?? 'لم يحدد'.tr())
                    : 'لم يحدد'.tr(),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child:widget.canedite==true? ElevatedButton.icon(
                        icon: Container(
                          padding: EdgeInsets.all(4),
                          decoration: BoxDecoration(
                            color: backgroundColor,
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: TextField(
                                readOnly:
                                    widget.canedite == true ? false : true,
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              'جميع المنتجات موجودة بالفعل في طلب الإنتاج الحالي'.tr(),
              style: GoogleFonts.cairo(
                fontSize: 14,
                color: lightTextColor,
              ),
  ...
  String _getCurrentOrderNameForBranch() {
    ...
    // البحث عن آخر طلب إنتاج للفرع المحدد
    for (var item in items) {
    ...
    return "طلب انتاج - ${DateFormat('dd/MM/yyyy - HH:mm').format(DateTime.now())}";
  }
...
if (widget.canedite == true && 
    !_isCurrentBranchSent())
                                Padding(
                                  padding: EdgeInsets.symmetric(
                                    horizontal: isVerySmallScreen
                                        ? 16.0
